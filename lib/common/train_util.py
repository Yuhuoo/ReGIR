...
def load_networks(cfg, model, mlp_path, high_voxel_path, normal_path):
    ...
    # high_voxel loading
    if os.path.exists(high_voxel_path) and high_voxel_path.endswith("ckpt"):
        high_dict = torch.load(high_voxel_path, map_location=torch.device(f"cuda:{cfg.gpus[0]}"))["state_dict"]

        high_dict = {
            k: v
            for k, v in high_dict.items() if k in high_dict and v.shape == high_dict[k].shape and
            ("reconEngine" not in k) and ("normal_filter" not in k) and ("voxelization" not in k)
        }
        print(colored(f"Resume high voxel weights from {high_voxel_path}", "green"))

    # normal network part loading
    if os.path.exists(normal_path) and normal_path.endswith("ckpt"):
        normal_dict = torch.load(normal_path, map_location=torch.device(f"cuda:{cfg.gpus[0]}"))["state_dict"]
