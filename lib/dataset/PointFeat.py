...
class PointFeat:
    ...
    def query(self, points, feats={}):
        ...
        # ipdb.set_trace()
        closest_triangles = torch.gather(self.triangles, 1, pts_ind[:, :, None, None].expand(-1, -1, 3, 3)).view(-1, 3, 3) # [35937, 3, 3]
        
        # ipdb.set_trace()
        bary_weights = barycentric_coordinates_of_projection(points.view(-1, 3), closest_triangles) # [35937, 3]
        
        # ipdb.set_trace()
        out_dict = {}

        for feat_key in feats.keys(): # ['smpl_verts', 'smpl_faces', 'smpl_sdf', 'smpl_cmap', 'smpl_norm']
            # [1, 6890, 3]  [1, 13776, 3] [1, 35937, 1] [1, 35937, 3] [1, 35937, 3] [1, 35937, 1])
            ...
            if feat_key in del_keys:
                ...
                closest_feats = torch.gather(feat_tri, 1, pts_ind[:, :, None, None].expand(-1, -1, 3, feat_dim)).view(-1, 3, feat_dim)
        ...
        if "sdf" in out_dict.keys():
            pts_dist = torch.sqrt(residues) / torch.sqrt(torch.tensor(3)) # sqrt([1, 35937]) / 1.7 # 1、计算点到面的距离
            
            # ipdb.set_trace()
            pts_signs = 2.0 * (check_sign(self.verts, self.faces[0], points).float() - 0.5) # 2、检查内部还是外部
            
            # ipdb.set_trace()
            pts_sdf = (pts_dist * pts_signs).unsqueeze(-1) # 3、求解得到有向距离场
        ...
        if "norm" in out_dict.keys(): # [1, 35937, 3]
            pts_norm = out_dict["norm"] * torch.tensor([-1.0, 1.0, -1.0]).to(self.device)
            
            #ipdb.set_trace()
            out_dict["norm"] = F.normalize(pts_norm, dim=2)

        if "cmap" in out_dict.keys(): # [1, 35937, 3]
            # ipdb.set_trace()
            out_dict["cmap"] = out_dict["cmap"].clamp_(min=0.0, max=1.0) # 范围限制函数
            # ipdb.set_trace()

        # ipdb.set_trace()
        for out_key in out_dict.keys():
            # print(out_key, out_dict[out_key].shape)
