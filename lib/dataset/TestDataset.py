...
class TestDataset():
    ...
    def compute_voxel_verts(self, body_pose, global_orient, betas, trans, scale):
        ...
        verts = np.concatenate([smpl_model.verts, smpl_model.verts_added], axis=0) * scale.item() + trans.detach().cpu().numpy()
        faces = np.loadtxt(osp.join(self.smpl_data.tedra_dir, 'tetrahedrons_neutral_adult.txt'), dtype=np.int32) - 1
