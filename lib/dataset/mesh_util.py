...
def load_checkpoint(model, cfg):
    ...
    if os.path.exists(cfg.high_resume_path) and cfg.high_resume_path.endswith("ckpt"):
        high_dict = torch.load(cfg.high_resume_path, map_location=device)['state_dict']

        high_dict = {
            k: v
            for k, v in high_dict.items() if k in model_dict and v.shape == model_dict[k].shape and
            ('reconEngine' not in k) and ("normal_filter" not in k) and ('voxelization' not in k)
        }
        print(colored(f"Resume High_res weights from {cfg.high_resume_path}", 'green'))
